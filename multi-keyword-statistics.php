...
*/
/*
Copyright (C) 2009-2010 akshara, (webmaster AT multi-keyword-statistics DOT net)
...
$ks_plugin_version = '4.7.15';

// Language
$ks_plugin_dir = basename (dirname (__FILE__));
load_plugin_textdomain ('multi-keyword-statistics', 'wp-content/plugins/' . $ks_plugin_dir, $ks_plugin_dir );
			...
			switch ($key) {
				...
				case 'archive_date':
					...
					if (strlen ($replacement) == 4)
						$replacement = __('Archive of', 'multi-keyword-statistics') . ' ' . $replacement;
					else if (strlen ($replacement) == 6)
						$replacement = __('Archive of', 'multi-keyword-statistics') . ' ' . date_i18n ('F/Y', mktime (0, 0, 0, intval (substr ($replacement, 4)), 1, intval (substr ($replacement, 0, 4))));
					else if (strlen ($replacement) == 8)
						$replacement = __('Archive of', 'multi-keyword-statistics') . ' ' . date_i18n (isset ($v['date_format']) ? $v['date_format'] : get_option ('date_format'), mktime (0, 0, 0, intval (substr ($replacement, 4, 2)), intval (substr ($replacement, 6)), intval (substr ($replacement, 0, 4))));
				...
				case 'pagination':
					if (isset ($wp_query->query_vars['paged']) && $wp_query->query_vars['paged'] > 0)
						$replacement = __('&ndash; page', 'multi-keyword-statistics') . ' ' . $wp_query->query_vars['paged'];
					else if (intval ($v['paged']) > 1)
						$replacement = __('&ndash; page', 'multi-keyword-statistics') . ' ' . $v['paged'];
...
function keyword_statistics_add_meta_tags () {
	...
	if ($options['dont_serve_metadata'] && !$options['testmode']) {
		...
		echo "\n<!-- metadata insertion deactivated - multi-keyword-statistics-plugin $ks_plugin_version http://www.111waystomakemoney.com/ -->\n";
		return;
	}
	echo "\n<!-- metadata inserted by multi-keyword-statistics-plugin $ks_plugin_version http://www.111waystomakemoney.com/ " . ($options['testmode'] ? '' : '-->') . "\n";
	if (is_404 ())
	...
	if (($meta['title'] || $wp_query->query_vars['paged']) && !$options['testmode'] && $options['serve_title']) {
		...
		if ($wp_query->query_vars['paged']) {
			...
			if (!$hasPagination)
				$meta['title'] .= ' ' . __('&ndash; page', 'multi-keyword-statistics') . ' ' . $wp_query->query_vars['paged'];
		}
		ob_clean ();
		ob_start ();
		echo $buffer;
...
function keyword_statistics_update_metadata () {
	...
	if (!isset ($_POST['autosave'])) {
		...
		if (!wp_verify_nonce ($_POST['ks_update_meta_nonce'], 'ks_update_meta_nonce'))
			die (__('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while updating the meta informations for the post!', 'multi-keyword-statistics'));
...
function post_keyword_statistics () {
	...
	<table class="ksmeta">
		<tr>
			<td>
				<label for="kslang"><?php _e('Language', 'multi-keyword-statistics') ?>:</label>
			...
			<td>
				<label for="ksfilter"><?php _e('Filter stopwords', 'multi-keyword-statistics') ?>:</label>
			...
			<td>
				<label for="kslines"><?php _e('Show first', 'multi-keyword-statistics') ?>:</label>
	...
	<table class="ksmeta">
		<tr<?php echo ((((is_author_not_admin() || is_contributor()) && $options['hide_fields_authors_cant_change']) || (is_admin() && $options['hide_fields_authors_cant_change_from_admin'])) && intval ($options['authors_can_set_individual_title']) != 1 ? ' style="visibility:hidden;display:none"' : '') ?>>
			<td style="vertical-align:top"><label for="kstitle"><?php _e('Title', 'multi-keyword-statistics') ?>:</label></td>
			<td>
		...
		<tr<?php echo ((((is_author_not_admin() || is_contributor()) && $options['hide_fields_authors_cant_change']) || (is_admin() && $options['hide_fields_authors_cant_change_from_admin'])) && intval ($options['authors_can_edit_keywords']) != 1 ? ' style="visibility:hidden;display:none"' : '') ?>>
			<td style="vertical-align:top"><label for="kskeywords"><?php _e('Keywords', 'multi-keyword-statistics') ?>:</label></td>
			<td>
				...
				<input type="checkbox" name="keywords_autoupdate" id="keywords_autoupdate" onchange="updateTextInfo()" value="ks_kau" <?php echo (($post->filter == 'edit' && intval ($meta['keywords_autoupdate']) == 1) || (($post->filter != 'edit' || !$meta) && $options['automatic_meta_data_update']) ? 'checked="checked"' : '') ?> /> <?php _e('use generated keywords', 'multi-keyword-statistics') ?>
				<?php } ?>
			</td>
		</tr>
		<tr<?php echo ((((is_author_not_admin() || is_contributor()) && $options['hide_fields_authors_cant_change']) || (is_admin() && $options['hide_fields_authors_cant_change_from_admin'])) && intval ($options['authors_can_control_robots']) != 1 ? ' style="visibility:hidden;display:none"' : '') ?>>
			<td><label for="ksrobotfollow"><?php _e('Robots', 'multi-keyword-statistics') ?>:</label></td>
			<td>
		...
		<tr<?php echo ((((is_author_not_admin() || is_contributor()) && $options['hide_fields_authors_cant_change']) || (is_admin() && $options['hide_fields_authors_cant_change_from_admin'])) && intval ($options['authors_can_edit_description']) != 1 ? ' style="visibility:hidden;display:none"' : '') ?>>
			<td style="vertical-align:top"><label for="ksdescription"><?php _e('Description', 'multi-keyword-statistics') ?>:</label></td>
			<td>
				...
				<input type="checkbox" name="description_autoupdate" id="description_autoupdate" onchange="updateTextInfo()" value="ks_dau" <?php echo (($post->filter == 'edit' && intval ($meta['description_autoupdate']) == 1) || (($post->filter != 'edit' || !$meta) && $options['automatic_meta_data_update']) ? 'checked="checked"' : '') ?> /> <?php _e('use generated description', 'multi-keyword-statistics') ?>
				<?php } ?>
			</td>
		</tr>
	</table>
		...
		if (rex.exec (element.id)) {
			...
			switch (type) {
				case 'title':
					...
					status = ptitle.length + '/' + '<?php echo $options['max_title_length'] . ' ' . __('characters used', 'multi-keyword-statistics') ?>';
					...
					status = status + '<br/><?php echo __('Preview', 'multi-keyword-statistics') ?>: ' + '<span style="color:#888"><?php echo $options['sitewide_title_predecessor'] ?></span>' + element.value + '<span style="color:#888"><?php echo $options['sitewide_title_successor'] ?></span>';
					<?php } ?>
					break;
				case 'keywords':
					...
					status = pk.length + '/' + '<?php echo $options['max_keywords_length'] . ' ' . __('characters', 'multi-keyword-statistics') ?>';
					var keys = pk.split (',');
					for (var i = 0; i < keys.length; i++)
						if (keys[i] == '')
							keys.splice (i, 1);
					status = status + ' - ' + keys.length + '/' + '<?php echo $options['max_keywords_count'] . ' ' . __('keywords / phrases used', 'multi-keyword-statistics') ?>';
				...
				case 'description':
					...
					status = status + '<br/><?php echo __('Preview', 'multi-keyword-statistics') ?>: ' + '<span style="color:#888"><?php echo $options['sitewide_description_predecessor'] ?></span>' + element.value + '<span style="color:#888"><?php echo $options['sitewide_description_successor'] ?></span>';
					<?php } ?>
					break;
			}
...
function post_keyword_statistics_div () {
	echo '<div class="dbx-b-ox-wrapper">' .
	     ...
	     __('Multi Keyword Statistics', 'multi-keyword-statistics') . "</h3></div>" .   
...
function keyword_statistics () {
	if (function_exists ('add_meta_box')) {
		...
		add_meta_box ('keywordstatbox', __('Multi Keyword Statistics', 'multi-keyword-statistics'), 'post_keyword_statistics', 'post', 'normal', 'core');
		add_meta_box ('keywordstatbox', __('Multi Keyword Statistics', 'multi-keyword-statistics'), 'post_keyword_statistics', 'page', 'normal', 'core');
	} else {
		// older versions
...
function ks_additional_options_menu () {
	...
	add_menu_page (__('Multi Keyword Statistics', 'multi-keyword-statistics'), __('Multi Keyword Statistics', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu');
	// sitewide predecessors and successors for title, description and keywords
	add_submenu_page ('ks-additional-menu', __('Sitewide Meta', 'multi-keyword-statistics'), __('Sitewide Meta', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu', 'ks_additional_options_sitewide_meta');
	// metadata for aggregations
	add_submenu_page ('ks-additional-menu', __('Aggregations Meta', 'multi-keyword-statistics'), __('Aggregations Meta', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu-aggregation-meta', 'ks_additional_options_aggregation_meta');
	// configuration for serving metadata
	add_submenu_page ('ks-additional-menu', __('Metadata Delivery', 'multi-keyword-statistics'), __('Metadata Delivery', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu-metadata', 'ks_additional_options_metadata');
	// automatic metadata generation
	add_submenu_page ('ks-additional-menu', __('Metadata Generation', 'multi-keyword-statistics'), __('Metadata Generation', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu-metadata-generation', 'ks_additional_options_metadata_generation');
	// sitewide default settings for controlling robots
	add_submenu_page ('ks-additional-menu', __('Robots Default', 'multi-keyword-statistics'), __('Robots Default', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu-robots', 'ks_additional_options_robots');
	// keyword density checker
	add_submenu_page ('ks-additional-menu', __('Keyword Density Checker', 'multi-keyword-statistics'), __('Keyword Density Checker', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu-keyword-density-checker', 'ks_additional_options_keyword_density_checker');
	// settings for displaying or allowing authors and contributors to change various fields
	add_submenu_page ('ks-additional-menu', __('Authors', 'multi-keyword-statistics'), __('Authors', 'multi-keyword-statistics'), 'administrator', 'ks-additional-menu-authors', 'ks_additional_options_authors');
}
...
function ks_additional_options_sitewide_meta () {
	$types = array ('title' => __('titles', 'multi-keyword-statistics'),
			'keywords' => __('keywords', 'multi-keyword-statistics'),
			'description' => __('descriptions', 'multi-keyword-statistics'));
	$options = keyword_statistics_get_plugin_configuration ();
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_sitewide_meta_nonce'], 'ks_sitewide_meta_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while saving sitewide meta informations!', 'multi-keyword-statistics');
		}
		else {
			// check values
					...
					switch ($matches[1]) {
						case 'title':
							if (strlen ($value) > $options['max_title_length']) {
								$err = 1;
								$errmsg = __('Too many characters used at the title-tag! See configuration at', 'multi-keyword-statistics');
								$errmsg .= ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a>. ';
						...
						case 'keywords':
							...
							if (count (explode (',', $value)) > intval ($options['max_keywords_count']) || strlen ($value) > $options['max_keywords_length']) {
								$err = 1;
								$errmsg = __('Too many characters and/or keywords/phrases used at the keywords tag! At', 'multi-keyword-statistics');
								$errmsg .= ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a> ';
								$errmsg .= __('you can change the configuration options.', 'multi-keyword-statistics');
							}
							break;
						case 'description':
							if (strlen ($value) > $options['max_description_length']) {
								$err = 1;
								$errmsg = __('The description you entered is too long. Have a look at', 'multi-keyword-statistics');
								$errmsg .= ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a> ';
		...
		else { ?> <div class="error"><p><strong><?php echo __('Options not saved', 'multi-keyword-statistics') . (isset ($errmsg) ? ' - ' . $errmsg : '!'); ?></strong></p></div> <?php }
	} ?>
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Sitewide Meta', 'multi-keyword-statistics') ?></h2>

<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/multi-keyword-statistics/" target="_blank">Plugin Page</a></h1>
		<form name="ks_options" method="post" action="">
			...
			<table class="form-table">
				...
				<tr valign="top">
					<th scope="row"><label for="ks_<?php echo $key ?>_successor"><?php echo __('Append after all', 'multi-keyword-statistics') . ' ' . $value ?>:</label></th>
					<td>
	...
	</div><h2 style="text-align: left">If u like the plugin please Donate:<a href="http://111waystomakemoney.com/donate/" target="_blank">Plugin Donation Page</a></h2> <?php
}

// options for defining meta informations of content aggregations
function ks_additional_options_aggregation_meta () {
	// aggregation types
	$aggregations = array ('home' => __('home', 'multi-keyword-statistics'),
	...
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_aggregation_meta_nonce'], 'ks_aggregation_meta_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while attempting to save aggregations meta information!', 'multi-keyword-statistics');
		}
		else {
					...
					switch ($matches[2]) {
						case 'title':
							...
							if (strlen ($cvalue) > $options['max_title_length']) {
								$err = 1;
								$errmsg = __('At section', 'multi-keyword-statistics') . ' "' . __('Metadata for', 'multi-keyword-statistics') . ' ' . __($aggregations[$matches[1]], 'multi-keyword-statistics') . '": ' . __('The title is containing too many characters!', 'multi-keyword-statistics');
								$errmsg .= ' ' . __('The options for setting the maximum length of this field you\'ll find at', 'multi-keyword-statistics');
								$errmsg .= ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a>.';
							}
							break;
						case 'keywords':
							...
							if (count (explode (',', $cvalue)) > intval ($options['max_keywords_count']) || strlen ($cvalue) > $options['max_keywords_length']) {
								$err = 1;
								$errmsg = __('At section', 'multi-keyword-statistics') . ' "' . __('Metadata for', 'multi-keyword-statistics') . ' ' . __($aggregations[$matches[1]], 'multi-keyword-statistics') . '": ' . __('Keywords field is containing too many characters or too many keywords/phrases are included!', 'multi-keyword-statistics');
						...
						case 'description':
							...
							if (strlen ($cvalue) > $options['max_description_length']) {
								$err = 1;
								$errmsg = __('At section', 'multi-keyword-statistics') . ' "' . __('Metadata for', 'multi-keyword-statistics') . ' ' . __($aggregations[$matches[1]], 'multi-keyword-statistics') . '": ' . __('The description is containing too many characters!', 'multi-keyword-statistics');
								$errmsg .= ' ' . __('The options for setting the maximum length of this field you\'ll find at', 'multi-keyword-statistics');
								$errmsg .= ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a>.';
							}
							break;
	...
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Aggregations Meta', 'multi-keyword-statistics') ?></h2>
			...
			<?php foreach ($aggregations as $key => $value) { ?>
				...
				<tr valign="top">
					...
					<td>
						...
						<?php _e('Preview', 'multi-keyword-statistics') ?>: <span id="ks_<?php echo $key ?>_title_preview"></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label for="ks_<?php echo $key ?>_keywords"><?php _e('Keywords', 'multi-keyword-statistics') ?>:</label></th>
					<td>
						...
						<?php _e('Preview', 'multi-keyword-statistics') ?>: <span id="ks_<?php echo $key ?>_keywords_preview"></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label for="ks_<?php echo $key ?>_description"><?php _e('Description', 'multi-keyword-statistics') ?>:</label></th>
					<td>
						...
						<?php _e('Preview', 'multi-keyword-statistics') ?>: <span id="ks_<?php echo $key ?>_description_preview"></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label for="ks_<?php echo $key ?>_index"><?php _e('Robots', 'multi-keyword-statistics') ?>:</label></th>
					<td>
						<input type="radio" name="ks_<?php echo $key ?>_index" id="ks_<?php echo $key ?>_index" value="default" <?php echo $options[$key . '_index'] == 'default' ? 'checked="checked"' : '' ?> /> <?php _e('default', 'multi-keyword-statistics') ?>
						<input type="radio" name="ks_<?php echo $key ?>_index" id="ks_<?php echo $key ?>_index" value="index" <?php echo $options[$key . '_index'] == 'index' ? 'checked="checked"' : '' ?> /> <?php _e('index', 'multi-keyword-statistics') ?>
						<input type="radio" name="ks_<?php echo $key ?>_index" id="ks_<?php echo $key ?>_index" value="noindex" <?php echo $options[$key . '_index'] == 'noindex' ? 'checked="checked"' : '' ?> /> <?php _e('noindex', 'multi-keyword-statistics') ?><br/>
						<input type="radio" name="ks_<?php echo $key ?>_follow" id="ks_<?php echo $key ?>_follow" value="default" <?php echo $options[$key . '_follow'] == 'default' ? 'checked="checked"' : '' ?> /> <?php _e('default', 'multi-keyword-statistics') ?>
						<input type="radio" name="ks_<?php echo $key ?>_follow" id="ks_<?php echo $key ?>_follow" value="follow" <?php echo $options[$key . '_follow'] == 'follow' ? 'checked="checked"' : '' ?> /> <?php _e('follow', 'multi-keyword-statistics') ?>
						<input type="radio" name="ks_<?php echo $key ?>_follow" id="ks_<?php echo $key ?>_follow" value="nofollow" <?php echo $options[$key . '_follow'] == 'nofollow' ? 'checked="checked"' : '' ?> /> <?php _e('nofollow', 'multi-keyword-statistics') ?><br/>
						<?php if ($key == 'home') { ?>
						<span class="description"><?php echo __('If you set this to default, the settings at', 'multi-keyword-statistics') . ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-robots') . '" title="">' . __('Robots Default', 'multi-keyword-statistics') . '</a> ' . __('will be used for this kind of content aggregation.', 'multi-keyword-statistics') ?></span>
						<?php } ?>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label for="ks_<?php echo $key ?>_index"><?php _e('Only for the first page', 'multi-keyword-statistics') ?>:</label></th>
					<td>
						...
						<?php if ($key == 'home') { ?>
						<span class="description"><?php echo __('If this is activated, the options above will only be served at the first page of the particular aggregation. The other pages of the aggregation are handled automatically (see settings at', 'multi-keyword-statistics') . ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a>).' ?></span>
						<?php } ?>
					</td>
				</tr>
			<?php } ?>
...
<h2 style="text-align: left">If u like the plugin please Donate:<a href="http://111waystomakemoney.com/donate/" target="_blank">Plugin Donation Page</a></h2>
		<p>
			<span style="font-weight:bold"><?php _e('Variables you can use and their values:', 'multi-keyword-statistics') ?></span>
			<style type="text/css">
			/* <![CDATA[ */
			...
			<table class="vars">
				<thead>
					<tr align="left">
						<th><?php _e('Variable', 'multi-keyword-statistics') ?></th>
						<th><?php _e('Value (example)', 'multi-keyword-statistics') ?></th>
						<th><?php _e('Description', 'multi-keyword-statistics') ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						...
						<td><?php _e('The blogs title as defined at the standard configuration. You can use this variable in <b>any</b> of the fields.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%siteurl%</td>
						<td><?php $var_siteurl = ks_replace_variables ('%siteurl%'); echo $var_siteurl ?></td>
						<td><?php _e('URL of your blog. Also usable at <b>any</b> of the fields above.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%category_name%</td>
						<td><?php $categories = get_categories (array ('orderby' => 'count', 'number' => 1)); if (is_array ($categories) && count ($categories) > 0) $var_category = ks_replace_variables ('%category_name%', array ('category_name' => $categories[key ($categories)]->name)); else $var_category = __('No categories assigned!', 'multi-keyword-statistics'); echo $var_category ?></td>
						<td><?php _e('Name of the currently listed category. Only replaced in <b>category</b> meta fields.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%tag_name%</td>
						<td><?php $tags = get_tags (array ('orderby' => 'count', 'number' => 1)); if (is_array ($tags) && count ($tags) > 0) $var_tag = ks_replace_variables ('%tag_name%', array ('tag_name' => $tags[key ($tags)]->name)); else $var_tag = __('No tags assigned!', 'multi-keyword-statistics'); echo $var_tag ?></td>
						<td><?php _e('Same as with category name but only on <b>tag</b> views.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%author_name%</td>
						<td><?php $var_author = ks_replace_variables ('%author_name%', array ('author_name' => 'Will Writer')); echo $var_author ?></td>
						<td><?php _e('Authors name when opening author aggregations. Only availiable at <b>author</b> meta definitions.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%archive_date%</td>
						<td><?php $var_archive = ks_replace_variables ('%archive_date%', array ('m' => '20100226')); echo $var_archive ?></td>
						<td><?php _e('This variable is only availiable at <b>archive</b> meta definitions and will be replaced only on archive-views of your blog.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%search_query%</td>
						<td><?php $var_search_query = ks_replace_variables ('%search_query%', array ('s' => __('how to use variables', 'multi-keyword-statistics'))); echo $var_search_query ?></td>
						<td><?php _e('Will be replaced with the search query on search result pages. Should only be used on meta configuration for <b>search results</b>.', 'multi-keyword-statistics') ?></td>
					</tr>
					<tr>
						<td>%pagination%</td>
						<td><?php $var_pagination = ks_replace_variables ('%pagination%', array ('paged' => 3)); echo $var_pagination ?></td>
						<td><?php _e('Pagination containing the page actually displayed. On the first page of an aggregation this will be deleted. You can use it in <b>any</b> field above.', 'multi-keyword-statistics') ?></td>
					</tr>
				</tbody>
			</table>
		</p>
		...
		function setPreview (element) {
			...
			if (document.getElementById (element.id + '_preview')) {
				switch (type) {
					case 'title':
						var len = ' (' + content_raw.length + '/<?php echo $options['max_title_length'] ?> <?php echo _e('characters used', 'multi-keyword-statistics') ?>)';
					...
					case 'keywords':
						...
						for (var i = 0; i < keys.length; i++)
							if (keys[i].replace (/\s/g, '').length == 0)
								keys.splice (i, 1);
						var keycount = ' (' + keys.length + '/<?php echo $options['max_keywords_count'] ?> <?php echo _e('keywords', 'multi-keyword-statistics') ?>' + ' - ' + content_raw.length + '/<?php echo $options['max_keywords_length'] ?> <?php echo _e('characters used', 'multi-keyword-statistics') ?>)';
						...
						break;
					case 'description':
						var len = ' (' + content_raw.length + '/<?php echo $options['max_description_length'] ?> <?php echo _e('characters used', 'multi-keyword-statistics') ?>)';
...
function ks_additional_options_authors () {
	...
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_author_options_nonce'], 'ks_author_options_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while saving plugin configuration for authors!', 'multi-keyword-statistics');
		}
		else {
	...
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Authors', 'multi-keyword-statistics') ?></h2>
<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/multi-keyword-statistics/" target="_blank">Plugin Page</a></h1>
		<form name="ks_options" method="post" action="">
			<input type="hidden" name="ks_update_options" value="1" />
			...
			<table class="form-table">
				...
				<tr valign="top">
					<th scope="row"><?php _e('Authors can', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_authors_can_change_content_language" value="1" <?php echo $options['authors_can_change_content_language'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('change the content language', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_authors_can_disable_stopword_filter" value="1" <?php echo $options['authors_can_disable_stopword_filter'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('switch status of the stopwords-filter', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_authors_can_set_individual_title" value="1" <?php echo $options['authors_can_set_individual_title'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('set individual title-tag', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_authors_can_edit_keywords" value="1" <?php echo $options['authors_can_edit_keywords'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('edit keywords', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_authors_can_edit_description" value="1" <?php echo $options['authors_can_edit_description'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('edit description', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_authors_can_control_robots" value="1" <?php echo $options['authors_can_control_robots'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('control robots', 'multi-keyword-statistics') ?><br/>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Hide fields authors can\'t change', 'multi-keyword-statistics') ?></th>
					<td>
						...
						<span class="description"><?php _e('Fields authors can\'t change may confuse them and is wasting space on screen. Activate this option to hide these fields.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Hide fields authors can\'t change from admin', 'multi-keyword-statistics') ?></th>
					<td>
						...
						<span class="description"><?php _e('Hide the fields for admins too. This will save space in page-/post-edit if the statistics tool is used for checking the keyword densities only.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
			</table>
			<p class="submit"><input type="submit" name="Submit" value="<?php _e('Update') ?>" /></p>
...
<h2 style="text-align: left">If u like the plugin please Donate:<a href="http://111waystomakemoney.com/donate/" target="_blank">Plugin Donation Page</a></h2> <?php
}
...
function ks_additional_options_robots () {
	...
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_robots_nonce'], 'ks_robots_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while saving configuration for controlling robot!', 'multi-keyword-statistics');
		}
		else {
			$options['noodp'] = $options['noydir'] = $options['noarchive'] = $options['index_aggregated'] = $options['follow_aggregated'] = 0;
			foreach ($_POST as $key => $value) {
				switch ($key) {
					case 'ks_noodp':
						$options['noodp'] = 1;
	...
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Robots Default Settings', 'multi-keyword-statistics') ?></h2>
<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/multi-keyword-statistics/" target="_blank">Plugin Page</a></h1>
		<form name="ks_options" method="post" action="">
			<input type="hidden" name="ks_update_options" value="1" />
			<input type="hidden" name="ks_robots_nonce" value="<?php echo wp_create_nonce ('ks_robots_nonce') ?>" />
			<table class="form-table">
				<tr><td colspan="2"><span class="description"><?php echo __('Here you can configure the default settings - index and follow - for the robots meta tag on content aggregations. These will be served for all aggregations that are not set to "default" at', 'multi-keyword-statistics') . ' <a href="' . admin_url ('admin.php?page=ks-additional-menu-aggregation-meta') . '" title="">' . __('Aggregations Meta', 'multi-keyword-statistics') . '</a> ' . __('configuation. You also can configure the settings for ODP, Yahoo! Directory and archiving (caching of your pages at search engines and archive.org) - ydir, odp and archive. These will be served with any page of your blog.', 'multi-keyword-statistics') ?></span></td></tr>
				<tr valign="top"><th colspan="2" style="padding-bottom:0;"><h3 style="margin-bottom:0;"><?php _e('Settings for Content Aggregations', 'multi-keyword-statistics') ?></h3></th></tr>
				<tr valign="top">
					<th scope="row"><?php _e('Robots', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_index_aggregated" value="1" <?php echo $options['index_aggregated'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('allow robots to index aggregated content', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_follow_aggregated" value="1" <?php echo $options['follow_aggregated'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('allow robots to follow links in aggregated pages', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('This options can be used to manipulate the robots meta tag in all kinds of content-aggregations (category-, tag-, archive-, author- and post-lists).', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top"><th colspan="2" style="padding-bottom:0;"><h3 style="margin-bottom:0;"><?php _e('Sitewide settings for ODP, Yahoo! Directory and archiving', 'multi-keyword-statistics') ?></h3></th></tr>
				<tr valign="top">
					<th scope="row"><?php _e('Robots', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_noodp" value="1" <?php echo $options['noodp'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('don\'t use information from ODP', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_noydir" value="1" <?php echo $options['noydir'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('don\'t use information from Yahoo! Directory', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_noarchive" value="1" <?php echo $options['noarchive'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('prevent archiving on archive.org and search engines', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('These settings have infuence on sitewide robots meta data (if checked noodp, noydir or noarchive will be used).', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
			</table>
			<p class="submit"><input type="submit" name="Submit" value="<?php _e('Update') ?>" /></p>
...
function ks_additional_options_metadata () {
	...
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_serve_meta_nonce'], 'ks_serve_meta_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while saving options for serving meta informations!', 'multi-keyword-statistics');
		}
		else {
	...
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Metadata Delivery', 'multi-keyword-statistics') ?></h2>
<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/multi-keyword-statistics/" target="_blank">Plugin Page</a></h1>
		<form name="ks_options" method="post" action="">
			<input type="hidden" name="ks_update_options" value="1" />
			<input type="hidden" name="ks_serve_meta_nonce" value="<?php echo wp_create_nonce ('ks_serve_meta_nonce') ?>" />
			<table class="form-table">
				<tr><td colspan="2"><span class="description"><?php _e('The settings below are used to configure which meta informations (tag) should be served by your blog. You can individually switch off or on particular tags or disallow the plugin to serve any meta information - e.g. if you use other plugins for serving meta informations instead and don\'t want to generate multiple tags of one type.', 'multi-keyword-statistics') ?></span></td></tr>
				<tr valign="top"><th colspan="2" style="padding-bottom:0;"><h3 style="margin-bottom:0;"><?php _e('Meta tag configuration', 'multi-keyword-statistics') ?></h3></th></tr>
				<tr valign="top">
					<th scope="row"><?php _e('Serve title tag', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_serve_title" value="1" <?php echo $options['serve_title'] == 1 ? 'checked="checked"' : '' ?> /><br/>
						<span class="description"><?php _e('If you don\'t want to let the plugin serve the page title-tag, you can deactivate this option. In this case the title generated by WordPress or another plugin will stay in the HTML code of each page.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Serve meta informations for', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_serve_meta_robots" value="1" <?php echo $options['serve_meta_robots'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('robots', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('If you want to use a robots.txt file to control robots you can uncheck this option. After that no robots meta will be embedded into the pages of your blog.', 'multi-keyword-statistics') ?></span><br/>
						<input type="checkbox" name="ks_serve_meta_keywords" value="1" <?php echo $options['serve_meta_keywords'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('keywords', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('Switch this off if you don\'t want your blog to serve the keywords generated automatic by the plugins Multi Keyword Statistics or your custom set of keywords set on each post or page. The keywords will also not be added to the HTML meta section on all kinds of content aggregations.', 'multi-keyword-statistics') ?></span><br/>
						<input type="checkbox" name="ks_serve_meta_description" value="1" <?php echo $options['serve_meta_description'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('description', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('Same as with the keywords option described before.', 'multi-keyword-statistics') ?></span><br/>
						<input type="checkbox" name="ks_serve_meta_canonical" value="1" <?php echo $options['serve_meta_canonical'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('canonical', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('Don\' let the plugin serve the canonical urls at any page served by the blog - including the various content aggregations - if this is off. In this case the canonical generation of WordPress 2.9+ will be reactivated. In older WordPress versions no canonical will be generated if you are not running another plugin doing this.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Don\'t serve any meta information', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_dont_serve_metadata" value="1" <?php echo $options['dont_serve_metadata'] == 1 ? 'checked="checked"' : '' ?> /><br/>
						<span class="description"><?php _e('This option overwrites the ones above so if you activate it no metadata will be served. If you are using other plugins for adding meta informations you can check this option. This way no metadata will be inserted at any page of your blog. The plugin can still be used to check keyword densities.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Activate testmode', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_testmode" value="1" <?php echo $options['testmode'] == 1 ? 'checked="checked"' : '' ?> /><br/>
						<span class="description"><?php _e('If activated the meta informations are served within a HTML comment. Search engines ignore these informations but while testing the plugins output you can see what will be served if you are not in testmode. This way you can compare the plugins metadata with the ones integrated by other plugins doing the same or similar work.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
			</table>
			<p class="submit"><input type="submit" name="Submit" value="<?php _e('Update') ?>" /></p>
...
function ks_additional_options_keyword_density_checker () {
	...
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_density_checker_nonce'], 'ks_density_checker_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while saving options for the keyword density checker!', 'multi-keyword-statistics');
		}
		else {
				...
				switch ($key) {
					...
					case 'ks_min_words':
						if (preg_match ('/[0-9]+/', $value))
							...
							$err = 1;
							//$errmsg = __('', 'multi-keyword-statistics');
						}
						break;
	...
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Keyword Density Checker', 'multi-keyword-statistics') ?></h2>
<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/multi-keyword-statistics/" target="_blank">Plugin Page</a></h1>
		<form name="ks_options" method="post" action="">
			<input type="hidden" name="ks_update_options" value="1" />
			<input type="hidden" name="ks_density_checker_nonce" value="<?php echo wp_create_nonce ('ks_density_checker_nonce') ?>" />
			<table class="form-table">
				<tr><td colspan="2"><span class="description"><?php echo __('With these options you can configure the keyword density checker. You can set a stopwords filter and in which iterval the statistics will be generated. These settings are affecting the statistics shown at edit page/post only. The settings for the automatic generation of meta informations can be found at ', 'multi-keyword-statistics') . '<a href="' . admin_url ('admin.php?page=ks-additional-menu-metadata-generation') . '" title="">' . __('Metadata Generation', 'multi-keyword-statistics') . '</a>.' ?></span></td></tr>
				<tr valign="top">
					<th scope="row"><?php _e('Default language', 'multi-keyword-statistics') ?></th>
					<td>
						...
						<span class="description"><?php _e('Default language for filtering stopwords', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Integrate content language in meta informations', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_set_language_meta" value="1" <?php echo $options['set_language_meta'] == 1 ? 'checked="checked"' : '' ?> /><br/>
						<span class="description"><?php _e('If your blog contains content in languages not listed in the selection above or you are planning to write posts or pages in not supported languages, you should deactivate this option. This will prevent from sending the false content language and let the client check it automatically.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Filter stopwords', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_filter_stopwords" value="1" <?php echo $options['filter_stopwords'] == 1 ? 'checked="checked"' : '' ?> />
						<span class="description"><?php _e('Activate the stopword-filter by default when post/page-editor will be opened', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Items in keyword lists', 'multi-keyword-statistics') ?></th>
					<td>
						<select name="ks_max_list_items">
							<?php for ($i = 1; $i < 11; $i++) echo '<option value="' . $i . '" ' . ($options['max_list_items'] == $i ? 'selected="selected"' : '') . '>' . $i . '</option>'; ?>
						</select>
						<span class="description"><?php _e('Display this number of the most common keywords / key-phrases', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Automatic Update', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_automatic_update" value="1" <?php echo $options['automatic_update'] == 1 ? 'checked="checked"' : '' ?> /><br/>
						<span class="description"><?php _e('Should the statistics refresh automatically? Turn off this option, if you have got slow JavaScript-Performance and use the button in the various edit-views instead!', 'multi-keyword-statistics') ?></span><br/>
						<?php _e('every', 'multi-keyword-statistics') ?> <select name="ks_update_interval">
							<?php for ($i = 1; $i < 11; $i++) echo '<option value="' . $i . '" ' . ($options['update_interval'] == $i ? 'selected="selected"' : '') . '>' . $i . '</option>'; ?>
						</select> <?php _e('seconds', 'multi-keyword-statistics') ?>
						<span class="description"><?php _e('Number of seconds between the updates', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Minimum number of words per post or page', 'multi-keyword-statistics') ?></th>
					<td>
						<select name="ks_min_words">
							<?php for ($i = 50; $i < 500; $i+= 10) echo '<option value="' . $i . '" ' . ($options['min_words'] == $i ? 'selected="selected"' : '') . '>' . $i . '</option>'; ?>
						</select><br/>
						<span class="description"><?php _e('If there is no content it doesn\'t make sense to generate meta informations from it. Here you can set the minimum number of words in a post or page that should be reached before keywords and description meta will be generated.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Keyword Phrases', 'multi-keyword-statistics') ?></th>
					<td>
						<input type="checkbox" name="ks_2word_phrases" value="1" <?php echo $options['2word_phrases'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('show 2-word phrases', 'multi-keyword-statistics') ?><br/>
						<input type="checkbox" name="ks_3word_phrases" value="1" <?php echo $options['3word_phrases'] == 1 ? 'checked="checked"' : '' ?> /> <?php _e('show 3-word phrases', 'multi-keyword-statistics') ?><br/>
						<span class="description"><?php _e('Switching on or off statistics for 2- and 3-word phrases (if your browser\'s JavaScript-Engine ist too slow and it is disturbing while writing)', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
			</table>
			<p class="submit"><input type="submit" name="Submit" value="<?php _e('Update') ?>" /></p>
...
function ks_additional_options_metadata_generation () {
	...
	if (isset ($_POST['ks_update_options']) && $_POST['ks_update_options'] == 1) {
		if (!wp_verify_nonce ($_POST['ks_meta_generation_nonce'], 'ks_meta_generation_nonce')) {
			$err = 1;
			$errmsg = __('Nonce verification failed:', 'multi-keyword-statistics') . ' ' . __('Error while saving options for automatic meta data generation!', 'multi-keyword-statistics');
		}
		else {
	...
	<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2><?php _e('Automatic Metadata Generation', 'multi-keyword-statistics') ?></h2>
...
<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/multi-keyword-statistics/" target="_blank">Plugin Page</a></h1>
		...
		<form name="ks_options" method="post" action="">
			...
			<table class="form-table">
				<tr><td colspan="2"><span class="description"><?php _e('Here you can configure if the Multi Keyword Statistics plugin should automatically set meta keywords and description of single posts or pages at edit-page/post. Moreover you can set some boundaries for the characters or keys used in the keywords and description meta tags. These values are used when the plugin is generating aggregated values for the tags. At edit-page/post these values are used to warn the author when he is using too many characters or words.', 'multi-keyword-statistics') ?></span></td></tr>
				<tr valign="top">
					<th scope="row"><?php _e('Update meta data automatically', 'multi-keyword-statistics') ?></th>
					<td>
						...
						<span class="description"><?php _e('Uncheck this option, if you want to edit the fields with the meta informations manually. If one of the fields is editable and has been edited, the automatic actualization will be disabled.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Words for aggregated descriptions', 'multi-keyword-statistics') ?></th>
					<td>
						...
						<span class="description"><?php _e('Set this to 0 if no automatic integration of a meta description is wanted in aggregation-views.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Maximum number of characters in title tags', 'multi-keyword-statistics') ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"><?php _e('Maximum number of keywords/phrases in keyword tags', 'multi-keyword-statistics') ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"><?php _e('Number of meta keywords generated in posts or pages', 'multi-keyword-statistics') ?></th>
					<td>
						...
						<span class="description"><?php _e('For generating a list of suggested keywords for setting the meta-keyword-tag of a single post or page.', 'multi-keyword-statistics') ?></span>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Maximum number of characters in keyword tags', 'multi-keyword-statistics') ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"><?php _e('Minimum number of characters in meta description', 'multi-keyword-statistics') ?></th>
					<td>
				...
				<tr valign="top">
					<th scope="row"><?php _e('Maximum number of characters in meta description', 'multi-keyword-statistics') ?></th>
					<td>
...
function keyword_statistics_admin_footer () {
	...
	function updateTextInfo () {
		...
		// output template
		var template = '<u><?php _e('Text Statistics', 'multi-keyword-statistics') ?></u>' +
			       '<table>' +
			       '<thead><tr><th>' + (document.getElementById ('ksfilter').checked ? '<?php _e('Total Words with / without Stopwords', 'multi-keyword-statistics') ?>' : '<?php _e('Total Words', 'multi-keyword-statistics') ?>') + '</th><th>' + (document.getElementById ('ksfilter').checked ? '<?php _e('Different Words with / without Stopwords', 'multi-keyword-statistics') ?>' : '<?php _e('Different Words', 'multi-keyword-statistics') ?>') + '</th><th><?php _e('Stopwords', 'multi-keyword-statistics') ?></th></tr></thead>' +
			       ...
			       '';//'<u><?php _e('Meta Keywords Suggestion', 'multi-keyword-statistics') ?></u>: [KEYWORDS:<?php echo $options['meta_keywords_count'] ?>]';
		...
		var t = new TextStatistics (textfield.value.replace (/\[caption.*caption=\"([^"]*)\"[^\]]*](.*)\[\/caption\]/ig, " $1 $2 "), lang);
		...
		if (m)
			for (var i = 0; i < m.length; i++) {
				...
				if (stats.keys.length > 0) {
					var table = '<u><br/>' + (keylength == 1 ? '<?php _e('Single Words', 'multi-keyword-statistics') ?>' : (keylength == 2 ? '<?php _e('2 Word Phrases', 'multi-keyword-statistics') ?>' : '<?php _e('3 Word Phrases', 'multi-keyword-statistics') ?>')) + '</u>' +
						    '<table><thead><tr><th style="width:6em;"><?php _e('Count', 'multi-keyword-statistics') ?></th><th style="width:10em;"><?php _e('Density', 'multi-keyword-statistics') ?></th><th><?php _e('Keyword / Phrase', 'multi-keyword-statistics') ?></th></tr></thead>' +
						    '<tfoot><tr><td colspan="3"><u><?php _e('Total / Different', 'multi-keyword-statistics') ?></u>: ' + stats.keycount + ' / ' + stats.different + '</td></tr></tfoot><tbody>';
					...
					template = template.replace (m[i], '<u>' + (keylength == 1 ? '<?php _e('No single keywords found', 'multi-keyword-statistics') ?>' : (keylength == 2 ? '<?php _e('No 2-word phrases found', 'multi-keyword-statistics') ?>' : '<?php _e('No 3-word phrases found', 'multi-keyword-statistics') ?>')) + '</u><br/><br/>');
			}
